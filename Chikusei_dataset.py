...
class TrainChikuseiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     img = np.float32(np.array(mat['crop_gt']['cube'])) # [c,w,h]
        #     img = np.transpose(img, (2, 1, 0)) # [c,w,h] -> [h,w,c]
        # mat.close()
        mat = loadmat(self.image_filenames[index])
        img = np.float32(np.array(mat['crop_gt']['cube'][0][0])) # [h,w,c]
...
# Test with image
class TestChikuseiDataset(Dataset):
    ...
    def __getitem__(self, index):
        # with h5py.File(self.image_filenames[index], 'r') as mat:
        #     img = np.float32(np.array(mat['crop_gt'][0,0]['cube'])) # [c,w,h]
        #     img = np.transpose(img, (2, 1, 0)) # [c,w,h] -> [h,w,c]
        # mat.close()
        mat = loadmat(self.image_filenames[index])
        img = np.float32(np.array(mat['crop_gt']['cube'][0][0])) # [h,w,c]
